...
$res = $lang->load("$extension", JPATH_ADMINISTRATOR, null, true, true) || $lang->load("$extension", $source, null, true, true) || $lang->load("$extension", JPATH_ADMINISTRATOR, $lang->getDefault(), true, true) || $lang->load("$extension", $source, $lang->getDefault(), true, true);
...
if ($arrayjson_live) {
    foreach ($arrayjson_live->{"hosts"} as $key => $value) {
      ...
      <td>" . $name . "</td>";
            if ($liveIsNow)
                $output .=" <td><a target='newTab' href='index.php?option=com_wimtvpro&view=wimlive_embedded&tmpl=component&id=" . $identifier . "' class='clickWebProducer' id='" . $identifier . "'><img src='components/com_wimtvpro/assets/images/webcam.png'></a></td>";
            else
                $output .="<td></td>";
      ...
      <td>" . $embedded_code . "</td>
        ...
        if (((isset($number))&&($number == "0")) && ($count == 0)) {
            break;
        }
    }
}
