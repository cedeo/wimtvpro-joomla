...
class WimtvproModelmymedias extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = array('id','state','position', 'title');
        }
...
//        var_dump($this->getState());
        ...
        $query->order($db->escape($this->getState('list.ordering', 'position')) . ' ' . $db->escape($this->getState('list.direction', 'ASC')));
