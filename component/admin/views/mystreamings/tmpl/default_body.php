...
foreach ($this->items as $i => $row) {
    ...
    <tr class="row<?php echo $k ?>">
        ...
        <td style="display:none;" class="center">
            <?php
//            echo JHtml::_('grid.id', $k, $item->id);
            echo JHtml::_('grid.id', $k, $row->id);
            ?>
        </td>
        ...
        <td><textarea style="margin: 0px; width: 165px; height: 70px;"><?php echo "[wimtv]" . $row->showtimeIdentifier . "|" . $width . "|" . $height . "[/wimtv]"; ?></textarea></td>


        <?php
        if (!isset($_GET["inserInto"])) {
            $statusPart1 = isset($status_array[1]) ? $status_array[1] : "";
            ?>
            <td><?php echo $action; ?></td>
            <td>
                <a class="jgrid" href="javascript:void(0);"  title="Download">
                    <a class="icon-32-download" href="<?php echo JURI::root() . "administrator/components/com_wimtvpro/includes/download.php?host_id=" . $row->contentidentifier ?>" id="<?php echo $row->contentidentifier . '|' . $statusPart1; ?>" rel="<?php echo $basePath; ?>">
                    </a>
                <!--<span class="icon-32-download" id="<?php // echo $row->contentidentifier . '|' . $statusPart1;          ?>" rel="<?php echo $basePath; ?>">
        ...
        if (isset($_GET["inserInto"])) {
            ...
            $iframeInsert = (isset($iframeInsert)) ? $iframeInsert : "";
            ?>
            <td>
                W (px) <input type="text" value="<?php echo $width ?>" id="width_<?php echo $row->contentidentifier; ?>">
                <br/><br/>H (px) <input type="text" value="<?php echo $height ?>"  id="height_<?php echo $row->contentidentifier; ?>"><br/>
                ...
                <br/>
                <a href="#" rel="<?php echo $row->contentidentifier; ?>" id='<?php echo $row->showtimeIdentifier; ?>' class="iframeClickedShortCode" onclick=""><?php echo JText::_('COM_MEDIA_INSERT_SHORTCODE') ?></a>
            </td>
        <?php } ?>
